...
class TaurusJDrawGraphicsFactory(Singleton, TaurusBaseGraphicsFactory, Logger):
    ...
    def getSceneObj(self,items):
        ...
        for item in items:
            print "\n item w getsceneobject: ", item
            try:
                if isinstance(item, Qt.QWidget):
                    print "\n instance of ", item, " = Qt.QWidget"
                    #scene.addItem(item)
    ...
    def getSwingObjectObj(self,params):
        ...
        elif className == "fr.esrf.tangoatk.widget.attribute.NumberScalarWheelEditor":
            item = self.getGraphicsItem('LineEdit', params)
            #s = params.get('summit')
            #x1, y1 = s[0], s[1]
            x1, y1, x2, y2 = params.get('summit')
            width = x2 - x1
